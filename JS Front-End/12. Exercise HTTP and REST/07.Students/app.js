    ...
    const loadStudents = async () => {
      ...
      try {
        ...
        for (const key in data) {
          ...
          td.appendChild(
            createElementWithTextContent("td", `${data[key].firstName}`)
          );
          td.appendChild(
            createElementWithTextContent("td", `${data[key].lastName}`)
          );
          td.appendChild(
            createElementWithTextContent("td", `${data[key].facultyNumber}`)
          );
          td.appendChild(
            createElementWithTextContent("td", `${data[key].grade}`)
          );
