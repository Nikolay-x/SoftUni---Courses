...
# oThe number consists of 7 digits (separated into two groups of 3 and 4 digits, respectively).
...
# regex = r"((^|(?<=[ ]))\+359 [2] \d+ \d{4})|((^|(?<=[ ]))\+359-[2]-\d{3}-\d{4}\b)"
# matches = re.finditer(regex, numbers_line)
#
...
regex = r"\+359 [2] \d+ \d{4}|\+359-[2]-\d{3}-\d{4}\b"
matches = re.findall(regex, numbers_line)
print(", ".join(matches))
