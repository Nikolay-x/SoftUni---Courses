...
# Submit your solutions in the SoftUni judge system at https://judge.softuni.org/Contests/Practice/Index/2303#2.
#
...
for i in range(heroes_number):
    ...
    heroes_dict[hero_name] = {"HP": hp}
    heroes_dict[hero_name].update({"MP": mp})
...
while True:
    ...
    if command[0] == "CastSpell":
        ...
        if heroes_dict[hero_name]["MP"] >= mp_needed:
            ...
            print(f"{hero_name} has successfully cast {spell_name} and "
                  f"now has {heroes_dict[hero_name]['MP']} MP!")
        else:
    ...
    if command[0] == "TakeDamage":
        ...
        if heroes_dict[hero_name]["HP"] > 0:
            print(f"{hero_name} was hit for {damage} HP by {attacker} and "
                  f"now has {heroes_dict[hero_name]['HP']} HP left!")
        else:
    ...
    if command[0] == "Recharge":
        ...
        if heroes_dict[hero_name]["MP"] >= 200:
            amount -= heroes_dict[hero_name]["MP"] - 200
    ...
    if command[0] == "Heal":
        ...
        if heroes_dict[hero_name]["HP"] >= 100:
            amount -= heroes_dict[hero_name]["HP"] - 100
