...
def move_rover(def_area_rows, def_area_cols, rover_r, rover_c, def_direction):
    if def_direction == "up":
        rover_r -= 1
        if rover_r < 0:
            rover_r = def_area_rows - 1

    elif def_direction == "down":
        rover_r += 1
        if rover_r > def_area_rows - 1:
            rover_r = 0

    elif def_direction == "left":
        rover_c -= 1
        if rover_c < 0:
            rover_c = def_area_cols - 1

    elif def_direction == "right":
        rover_c += 1
        if rover_c > def_area_cols - 1:
            rover_c = 0
...
for i in range(area_rows):
    ...
    if rover in row:
        rover_row = i
        rover_col = row.index(rover)
    area.append(row)

directions = input().split(", ")
for direction in directions:
    rover_row, rover_col = move_rover(area_rows, area_cols, rover_row, rover_col, direction)
    if area[rover_row][rover_col] == "W":
        print(f"Water deposit found at ({rover_row}, {rover_col})")
        resources_found.add("W")
    elif area[rover_row][rover_col] == "M":
        print(f"Metal deposit found at ({rover_row}, {rover_col})")
        resources_found.add("M")
    elif area[rover_row][rover_col] == "C":
        print(f"Concrete deposit found at ({rover_row}, {rover_col})")
        resources_found.add("C")
