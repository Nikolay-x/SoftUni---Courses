...
# Submit your solutions in the SoftUni judge system at https://judge.softuni.org/Contests/Practice/Index/3426#0
...
# name: str
# oIf it is an empty string, raise ValueError with the message "Name cannot be an empty string."
# energy: int
# oIf it is a negative number, raise ValueError with the message "Energy cannot be less than zero."
# Methods
...
# The class should have the following attributes:
# name: str
# oIf it's set to an empty string, raise ValueError with the message "Name not valid!"
# oThere should not be two players with the same name (they all should be unique). If a second player is created with the same name, raise Exception with the message "Name {name} is already used!"
# age: int
# oIf the player is under 12 years old, raise ValueError with the message "The player cannot be under 12 years old!"
# stamina: int
# oAn optional parameter, 100 by default
# oStamina's max value is 100, and its min value is 0
# oIf it is less than zero or more than 100, raise ValueError with the message "Stamina not valid!"
# need_sustenance: bool
# oReturns if the player's stamina is less than 100. It is read-only, and it should not be able to be set
# Methods
...
# players: list
# oAn empty list that will contain all the players (objects)
# supplies: list
# oAn empty list that will contain all the supplies (objects)
# Methods
# __init__()
# Upon initialization, all the needed attributes must be set.
