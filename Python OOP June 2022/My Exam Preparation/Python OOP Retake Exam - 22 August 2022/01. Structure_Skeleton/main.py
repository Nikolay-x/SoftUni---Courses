...
# phone_number: str
# oA string that represents the phone number of the client
# oIt must start with "0" (zero), must be 10 characters long, and must contain only numbers.
# Otherwise, raise a ValueError with the message "Invalid phone number!"
# shopping_cart: list
# oAn empty list that will contain all meals (objects) added by the client
# bill: float
# oIt represents the total amount of money for all meals that the client has added to his shopping cart
# oIt should be set to 0.0 initially
# Methods
...
# The class should have the following attributes:
# name: str
# oA string that represents the name of the meal
# oIf the name is an empty string, raise a ValueError with the message "Name cannot be an empty string!"
# price: float
# oA float number that represents the price per one piece of a meal
# oIf the price is less than or equal to 0.0, raise a ValueError with the message "Invalid price!"
# quantity: int
# oAn integer that represents the quantity available (to be ordered) for that meal
# Methods
...
# The class should have the following attributes:
# menu: list
# oAn empty list that will contain all the meals (objects)
# clients_list: list
# oAn empty list that will contain all the clients (objects)
# Methods
# __init__()
# In the __init__ method all the needed attributes must be set.
...
# oThe meal names refer to all meal names on the client's shopping cart. They should be separated by a comma and a space ", ".
# oThe bill is the total amount of money for all meals on the client's shopping cart. It should be formatted to the second decimal point.
...
# oIf the exception is raised, the client could NOT make the order at all (none of the meals should be added to the client's shopping cart, and the bill should not be increased).
...
# oThe meal types are "Starter", "MainDish" and "Dessert"
# oIf the exception is raised, the client could NOT make the order at all (none of the meals should be added to the client's shopping cart, and the bill should not be increased).
...
# oThe receipt_id is automatically generated each time a bill is paid (regardless of the client). It starts from 1 and increases by 1 with each new paid bill.
# oThe total paid money should be formatted to the second decimal place.
# If there are no meals on the client's shopping cart, raise an Exception with the message "There are no ordered meals!"
# __str__()
...
# print(food_orders_app.register_client("089999999"))
# print(food_orders_app.register_client("089999999a"))
# print(food_orders_app.register_client("08999999999"))
...
# tortilla_with_beef = "Tortilla with Beef, 15, 18"
...
print(food_orders_app.add_meals_to_menu(
    ...
    chocolate_and_violets))
print(food_orders_app.show_menu())
print(f"{', '.join([str(m.quantity) for m in food_orders_app.menu])}")
...
print(food_orders_app.add_meals_to_shopping_cart('0899999999', **food))
print(f"{', '.join([str(m.quantity) for m in food_orders_app.menu])}")
print(f"{', '.join([str(m.quantity) for m in food_orders_app.clients_list[0].shopping_cart])}")
